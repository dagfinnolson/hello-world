...
public class generirane {
    ...
    private boolean moveToCell(int deltaRow,
                               int deltaColumn) {
        int newRow = playersCurrentRow + deltaRow;
        int newColumn = playersCurrentColumn + deltaColumn;
        if (maze[newRow][newColumn] != 'X') {
            swapCells(playersCurrentRow, newRow,
                    playersCurrentColumn, newColumn);
            playersCurrentRow = newRow;
            playersCurrentColumn = newColumn;
            playersMovesCount++;
            return true;
        }
	...
	public  void movePlayer(char firstLetter){
        switch (toUpperCase(firstLetter)) {
        case 'L':
            if (!moveToCell(0, -1)){
